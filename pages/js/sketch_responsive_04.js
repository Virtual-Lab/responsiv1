...
function oscInit() {
  ...
  client.on('message', function(address, args) {
    ...
    if (address == "/kick") {
      ...
      if (obj == 1 || obj == 2) {
      tone[zaehler[obj]][obj].play(freq[obj]*3, vol[obj], dur[obj]);
      zaehler[obj]=(zaehler[obj]+1)%poly;
      console.log("Play Tone")
      };

      console.log("received message: ", address, obj, freq[obj], vol[obj], dur[obj]);      
    };
    ...
    if (address == "/klank") {
      ...
      if (obj == 1 || obj == 2) {
      //tone[zaehler[obj]][obj].adsr(dur[obj]/8, dur[obj]/4, vol[obj]/3*2, dur[obj]/2, 1, dur[obj]);
...
//      
    ...
    if (obj == 2) {
        ...
        var x1 = (dw/2)-10*variation[obj][0];
        if (x1<0) {x1 = x1+2; variation[obj][0]--};
        if (x1 > (2*dw/3)) {x1 = x1-2; variation[obj][0] = variation[obj][0]+1};
        
        fill(color);
          
        rect(x1, (dh/10),((dw)/variation[obj][0]), dh/(1+vol));
        rotactiv=true;
...
function klank(obj, freq, vol, dur) {
  ...
  var farbe = vol  * 800;
  var position = freq/6;
   console.log("klank: ", farbe, position);
...
function dodec(obj, vol, freq, dur) {

  console.log("dodec: ", obj, freq, vol, dur);

  var farbe = Math.abs(vol+1)/2  * 300;
  var position = freq;
    ...
    if (position == 0) {} else {
          ...
          if (obj == 1) {
              ...
              fill(farbe);
              noStroke();
              rect((dh/2)*position*10, 0,(dh)/(50+variation[obj][0]), dh);
          ...
          if (obj == 2) {
             ...
             for (var i = 0; i < objectcopies; i++) {
               ...
               noStroke();
               fill(farbe);
               triangle(dh/(Math.log(variation[obj][i])), (dh/2), 80, dh/(1+position), 50 + variation[obj][i], 50 + variation[obj][i]);
             };
            };
            ...
            if (obj == 4) {
               ...
               for (var i = 0; i < (dh); i++) { 
                 ...
                 for (var k = 0; k < objectcopies; k++) {
                   ...
                   siney[i] = (50*variation[obj][k]) * sin(i*2*Math.PI);
